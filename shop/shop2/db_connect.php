...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 同じ時間と日付に予約が入っているか確認
    $stmt = $db->prepare("SELECT COUNT(*) FROM reservations2 WHERE time = :time AND date = :date");
    ...
    // データベースにデータを挿入する処理
    $stmt = $db->prepare("INSERT INTO reservations2 (name, time, people_count, date) VALUES (:name, :time, :people_count, :date)");
