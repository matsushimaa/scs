...
$stmt = $db->query("SELECT time, SUM(people_count) AS total_people FROM reservations2 GROUP BY time ORDER BY time ASC");
...
foreach ($all_times as $time) {
    ...
    foreach ($reservations2 as $reservation2) {
        // 時間範囲を分割
        list($start_time, $end_time) = explode('~', $reservation2['time']);
        $start_time = trim($start_time);
        $end_time = trim($end_time);

        // 現在の時間がこの範囲にあるかを確認
        if ($time >= $start_time && $time < $end_time) {
            $availability[$time]['total_people'] += $reservation2['total_people'];
        }
    }
}
?>
