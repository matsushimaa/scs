...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 時間帯に対する予約人数を再確認
    $stmt = $db->prepare("SELECT SUM(people_count) AS total_people FROM reservations3 WHERE time = :time");
    ...
    if (($total_people + $people_count) > 2) {
        ...
        // データベースにデータを挿入する処理
        $stmt = $db->prepare("INSERT INTO reservations3 (name, time, people_count) VALUES (:name, :time, :people_count)");
